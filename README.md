...
| Artifact | Contents |
|----------|----------|
| `*_MASTER_RESULTS*.csv` | All detected spots from every image (full spot table for replotting and downstream stats) |
| `*_analysis.csv` | Per-image spot table (same columns as master, one file per image under `<dataset>/`) |
| `*_FILE_STATS*.csv` | Per-image class counts, NMJ formation rate, zone areas/densities (Friedman panels) |
...
#### `*_INTENSITY_PAIRED_IMAGES*.csv` (paired intensity cohort)

Despite the filename, this is a **spot table**, not a one-row-per-image list. Each row is one BTX punctum from fields that contain **both** early NMJ-like **and** Orphaned spots — the same image set used for the paired Wilcoxon signed-rank test (early NMJ-like vs Orphaned brightness within the same image).

**Cohort rule:** An image is included when it has ≥1 early NMJ-like spot and ≥1 Orphaned spot. All spots from those images are exported (Muscle-associated and Neuron-associated spots in the same field are included too).

**Prefixes:** `ALL_FOLDERS_INTENSITY_PAIRED_IMAGES*` (batch across datasets) or `BATCH_INTENSITY_PAIRED_IMAGES*` (current folder only).

| Column | Meaning |
|--------|---------|
| `SOURCE_FOLDER`, `SOURCE_IMAGE` | Dataset folder and image filename |
| `SPOT_ID` | Spot index within the image |
| `MEAN_INTENSITY` | Mean BTX fluorescence (A.U.) |
| `BTX signal class` | Spatial class (early NMJ-like, Orphaned, …) |
| `is_NMJ` | Whether the spot overlaps the functional NMJ mask |
| `AREA_PX`, `ROUNDNESS` | Spot morphology |
| `INNERVATION_OVERLAP_PCT` | Overlap with innervation mask (%) |
| `Dist_to_Muscle_um`, `Dist_to_Neuron_um` | EDT distance to muscle / neuron masks |
| `GLOBAL_BTX_INTENSITY_OTSU` | Batch-wide Otsu on **all** spots (vertical line on dashboard panel **01a**) |
| `PAIRED_BTX_INTENSITY_OTSU` | Otsu recomputed on **this cohort only** (reference for panel **01b**; may differ slightly from global) |

**Uses:**

- Right-hand **KDE** in row 0 of `*_SUMMARY*.png` (**panel 01b**) plots exactly these spots with the **paired** Otsu line; the table below it compares spot counts above global vs paired Otsu by class. Left panel (**01a**) uses all spots from `*_MASTER_RESULTS*.csv` with the global Otsu line and per-class histograms.
- Replot or re-export panel 01b from this CSV without re-reading raw images.
- Relative path is recorded in `run_config.json` as `paired_intensity_images_csv`.
- **Inference:** the paired Wilcoxon in `*_STAT_SUMMARY*.csv` runs on **per-image class medians** from `*_IMAGE_LEVEL_MEDIANS*.csv`, not on these spot rows directly. Use the spot table for visualization and replotting; cite the Wilcoxon row for statistics.

A descriptive global-vs-paired Otsu comparison row (`level=descriptive`) is also appended to `*_STAT_SUMMARY*.csv`.

#### Other key CSVs

| File | Role |
|------|------|
| `*_MASTER_RESULTS*.csv` | Complete spot inventory. Leading columns: `SOURCE_FOLDER`, `SOURCE_IMAGE`, `TOTAL_IMAGE_AREA_um2`, then per-spot metrics. Source for aggregate dashboards and spot-pooled exploratory tests. |
| `*_IMAGE_LEVEL_MEDIANS*.csv` | Long format: one row per image × class × metric (`Dist_to_Muscle_um`, `Dist_to_Neuron_um`, `MEAN_INTENSITY`, `ROUNDNESS`). Primary Kruskal–Wallis and Wilcoxon tests use these medians. |
| `*_STAT_SUMMARY*.csv` | One row per test. Filter on `level` for publication tables (`primary_image_level`, `primary_posthoc`). |
| `*_FILE_STATS*.csv` | Per-image zone partitioning and class counts; required to regenerate Friedman / zone-abundance panels. |
...
| Row | Left | Right |
|-----|------|-------|
| 0 | BTX intensity KDE + global Otsu + per-class histograms — **panel 01a** | Paired-cohort KDE with **paired** Otsu — **panel 01b** (top); global vs paired Otsu spot-count table (bottom right) |
...
The paired Wilcoxon uses only images that contain both early NMJ-like and Orphaned spots. Spot-level rows from that cohort are saved in `*_INTENSITY_PAIRED_IMAGES*.csv` (see **Output file details** above). Panel **01a** (left) shows all spots with the global Otsu line and per-class histograms; panel **01b** (right) shows the paired-cohort KDE with the **paired** Otsu line and a spot-count comparison table (`*_PAIRED_OTSU_SPOT_CHANGE*.csv`).
